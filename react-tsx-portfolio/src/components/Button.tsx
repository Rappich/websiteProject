...
import clsx from 'clsx'; // Import clsx helper

// Define expected properties for the button
interface ButtonProps {
    variant: 'primary' | 'secondary'; // Can only be one of these
...
const Button: React.FC<ButtonProps> = ({
    ...
    const baseStyles = 'font-semibold transition-all duration-300 ease-in-out p-4 w-32 rounded-full border-[0.1rem]'; // Adjusted ease
    ...
    if (href) {
        return (
            <a
                ...
                target={target}
                rel={rel || (target === '_blank' ? 'noopener noreferrer' : undefined)} // Add rel for target="_blank"
                onClick={onClick as any} // Cast needed if onClick expects button event with anchor
            >
                {children}
            </a>
        );
